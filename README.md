...
一句话：**一套经过真实任务验证的 Multica 小队配置**——每个 Agent 只负责一件事，Leader 负责编排与判门，每步产出都要证据。
...
👉 **[`templates/chinese/squad/software-development`](./templates/chinese/squad/software-development)**
...
在 Multica 创建 5 个 Agent（命名遵循 [`docs/chinese/naming-conventions.md`](./docs/chinese/naming-conventions.md)），把 [`templates/chinese/agents/`](./templates/chinese/agents/) 下对应文件的代码块复制到各自 Instructions：
...
| Skill | 来源 | 挂给谁 |
| --- | --- | --- |
| `multica-verification`（判门，必备） | [`templates/chinese/skills/multica-verification/SKILL.md`](./templates/chinese/skills/multica-verification/SKILL.md) | **Leader** |
| `multica-gate-setup` | [`templates/chinese/skills/multica-gate-setup/SKILL.md`](./templates/chinese/skills/multica-gate-setup/SKILL.md) | Leader（集成 CI 硬门禁时） |
| `multica-test-design` | [`templates/chinese/skills/multica-test-design/SKILL.md`](./templates/chinese/skills/multica-test-design/SKILL.md) | Tester |
| `multica-requirement-analysis` | [`templates/chinese/skills/multica-requirement-analysis/SKILL.md`](./templates/chinese/skills/multica-requirement-analysis/SKILL.md) | Leader / Architect |
| `multica-technical-design` | [`templates/chinese/skills/multica-technical-design/SKILL.md`](./templates/chinese/skills/multica-technical-design/SKILL.md) | Architect |
| `multica-implementation` | [`templates/chinese/skills/multica-implementation/SKILL.md`](./templates/chinese/skills/multica-implementation/SKILL.md) | FrontendDev / BackendDev |

> 6 个 Skill 全部共享放在 `templates/chinese/skills/`，统一 `multica-` 前缀命名空间（multica-verification 是判门，Bug Fix 也在用；multica-gate-setup 用于把 CI 硬门禁装进仓库并在判门时感知 CI 结论；multica-test-design 给 Tester 生成用例与测试报告）。Skill 靠**名称**挂载，谁需要就在自己的 Instructions 里写「用 xxx skill」，与仓库路径无关。

### Step 3 — 创建 Squad

创建 Squad，把 `templates/chinese/squad/software-development/squad.md` 复制到 Squad Instructions。

### Step 4 — 创建 Issue

把 `templates/chinese/squad/software-development/issue.md` 复制到新 Issue，填上你的需求。
...
```text
Issue → [设计] → [API 契约 ∥ 功能用例] → [前端 ∥ 后端实现] → [测试] → Human
```
...
| Starter | 用途 | 状态 |
| --- | --- | --- |
| [Software Development](./templates/chinese/squad/software-development) | 常规功能开发（前后端按范围路由，任意角色可缺失） | 推荐 |
| [Bug Fix](./templates/chinese/squad/bug-fix) | 根因 / 修复 / 回归（按影响面路由，跳过 Architect） | 实验性 |
...
| 文档 | 内容 |
| --- | --- |
| [where-to-put-things](docs/chinese/where-to-put-things.md) | 指令归属速查表（最值得读） |
| [gates-and-evidence](docs/chinese/gates-and-evidence.md) | 门禁 G0–G4 与证据要求 |
| [common-mistakes](docs/chinese/common-mistakes.md) | Bad → Good 错误示范 |
| [adapt-and-scale](docs/chinese/adapt-and-scale.md) | 裁剪、扩展、试点推广 |
| [naming-conventions](docs/chinese/naming-conventions.md) | Agent 命名规范（角色+姓名） |
