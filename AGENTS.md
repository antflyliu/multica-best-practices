...
├── chinese/         Chinese templates (default)
...
docs/                Methodology (split by language: chinese/ + english/)
...
- **Language directories**: `templates/` and `docs/` each contain a `chinese/` and an `english/` directory with the same file names (e.g. `docs/chinese/where-to-put-things.md` ↔ `docs/english/where-to-put-things.md`).
...
- **Links are language-aware**: inside an English file, relative links point into `english/` trees; the Chinese tree is never moved or rewritten for translation purposes.
- **No build tooling**: this is a plain Markdown repo — do not add a docs generator or symlinks.
- **Machine files are not translated**: `LICENSE`, `*.yml`, `*.json`, `*.sh` (CI gate files are copied into both `chinese/` and `english/` skill directories).
...
- **Keep starters copy-paste-ready**: `templates/chinese/squad/software-development` is the MVP; changes must not break "copy → paste → run".
