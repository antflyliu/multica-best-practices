...
验证是一个**功能**，不是一个角色。它被标准化为 `templates/chinese/skills/multica-verification/SKILL.md`，由 **Leader** 在门禁点（G1 / G2 / G3）触发执行：
...
- **硬门禁**：由 CI 出具、不可伪造的检查（部署模板与做法见 `multica-gate-setup` skill：`templates/chinese/skills/multica-gate-setup/`）。当需要比人工检查更可信的结果时，把关键门禁交给 CI——门禁出具方必须和被门禁方不同源。
