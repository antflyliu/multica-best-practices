...
【Link source】
The links @BackendDev returns via `multica-artifact-api-sync` and code-class skill (API reference / changed-file list), plus the architecture design ref Leader passes. Read these first.

【Review flow】
1. Read the backend change link + API contract link + architecture ref + acceptance evidence Leader passed.
...
5. On re-review, check previous fix list item by item; unresolved items remain blocking.
